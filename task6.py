...
add_list = [('Printer', 'HP LaserJet', 300, 'B&W', 2), ('Scanner', 'Canon lide 400', 100, False, 1),
            ...
            ('Printer', 'HP laserJet', 300, 'Color', 2), ('Printer', 'Canon ip6500', 400, 'Color', 1)]
w = Warehouse()
for i in add_list:
    for n in range(i[-1]):
        c = ddct[i[0]](i[1], i[2], i[3])
        w.add(c)
print('Заказан: Принтер для бухгалтерии, черно-белый. ', end='')
w.distribute('Printer', 'accounts', 'B&W')
print('Заказан: Принтер для маркетинга, цветной. ', end='')
